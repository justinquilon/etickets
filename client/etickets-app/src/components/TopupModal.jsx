...
function TopupModal(props) {
  ...
  const addLoadHandler = async (e) => {
    e.preventDefault();
    try {
      const loadApi = await fetch(
        'https://server-etickets.onrender.com/api/v1/accounts/load',
        {
          method: 'POST',
  ...
  return (
    ...
    >
      <Modal.Header closeButton>
        <Modal.Title id="contained-modal-title-vcenter">Topup Load</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <Form>
          ...
          <Form.Group className="mb-3" controlId="formTopup">
            <Button variant="primary" type="submit" onClick={addLoadHandler}>
              Submit
            </Button>
            <Button onClick={props.onHide} style={{ marginLeft: '10px' }}>
              Close
            </Button>
          </Form.Group>
        </Form>
